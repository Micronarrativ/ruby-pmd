...
class Pdfmdrename < Pdfmd
  ...
  attr_accessor :filename, :dryrun, :allkeywords, :outputdir, :nrkeywords, :copy, :filetarget
  ...
  def initialize
    ...
    hierakeymapping = Pdfmdmethods.determineValidSetting(nil, 'rename:keys', @@hieradata)
    hierakeymapping ?  @@keymapping = hierakeymapping : ''

    @defaultDoctype = Pdfmdmethods.determineValidSetting('doc', 'rename:defaultdoctype', @@hieradata)
    @fileextension  = 'pdf'
  end
  ...
  def rename
    ...
    puts 'new file: ' + @filetarget
    if @dryrun # Do nothing on dryrun
      if @filename == @filetarget
        Pdfmdmethods.log('info', "Dryrun: File '#{@filename}' already has the correct name. Doing nothing.")
      else
        Pdfmdmethods.log('info',"Dryrun: Renaming '#{@filename}' to '#{get_filename(newFilename)}'.")
      end
    elsif @filename == @filetarget # Do nothing when name is already correct.
      Pdfmdmethods.log('info',"File '#{@filename}' already has the correct name. Doing nothing.")
    else
      Pdfmdmethods.log('info',"Renaming '#{@filename}' to '#{@filetarget}'.")
      command     = command + " '#{@filename}' #{@filetarget} 2>/dev/null"
      system(command)
      if !$?.exitstatus
        Pdfmdmethods.log('error', "Error renaming '#{@filename}' to '#{@filetarget}'.")
        abort
      else
        Pdfmdmethods.log('info', "Successfully renamed file to '#{@filetarget}'.")
      end
  ...
  def get_outputdir(outputdir = '')
    ...
    if !outputdir # outputdir is set to false, assume pwd
      Pdfmdmethods.log('debug','No outputdir specified. Taking current pwd of file.')
      ...
      Pdfmdmethods.log('error',"Output directory '#{outputdir}' not accessible. Abort.")
      exit 1
    elsif outputdir and File.exist?(outputdir)
      outputdir
    else
  ...
  def get_keywords(preface = '')
    ...
    if !@@metadata['keywords'].empty?
      ...
      if @@metadata['keywords'] = !@allkeywords
        keywords = keywordsarraySorted.values_at(*(0..@nrkeywords-1)).join('_')
      else
        keywords = keywordsarraySorted.join('_')
      end

      # Normalize all keywords and return value
      I18n.enforce_available_locales = false
      I18n.transliterate(keywords).downcase.chomp('-').chomp('_')
